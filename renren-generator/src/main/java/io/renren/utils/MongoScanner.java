...
public class MongoScanner {
  ...
  /**
   * 如果一个文档是对象类型  获得这个属性的下一级的属性名的集合 例子: user:{name:"张三",age:12}  传入user  返回[name,age]
   *
   * @param parameterName 上层参数名  这个参数名可以包含一个或多个. 注: 参数传递之前需确认:  1.上层属性一定是对象类型
  ...
  public Set<String> getNextParameterNames(String parameterName) {
    ...
    Document project = new Document("$project",
        new Document("list", "$" + parameterName).append("_id", false));
    Document unwind2 = new Document("$unwind", "$list");
    AggregateIterable<Document> aggregate = this.collection
        .aggregate(Arrays.asList(match, unwind, limit, project, unwind2));
  ...
  /**
   * 功能描述:提供属性名 解析属性类型 获取相应的属性信息  封装成generator对象
   *
  ...
  public MongoDefinition processNameType(String propertyName) {
    ...
    MongoCursor<Document> isArray = collection
        .find(new Document(propertyName, new Document("$type", ARRAY_TYPE))).limit(1).iterator();
    if (isArray.hasNext()) {
      result.setArray(true);
      for (int i : TYPE) {
        MongoCursor<Document> iterator = collection
            .find(new Document(propertyName, new Document("$type", i))).limit(1).iterator();
        if (iterator.hasNext()) {
          if (i == 3) {
  ...
  private List<MongoDefinition> produceChildList(String parentName) {
    ...
    List<String> collect = strings.stream().map(name -> parentName + "." + name)
